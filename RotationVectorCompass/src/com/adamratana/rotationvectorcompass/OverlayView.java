...
public class OverlayView extends View {
	...
	/** 
	 * Lock used to mitigate between objects being rotated on a secondary thread (methods called by motion event listeners), and objects being displayed on the screen.
	 * we draw based on published values, and we lock while publishing those values, or drawing those published values, so values are not updated in mid-draw.
	 *   
	 */
	...
	/**
	 * handle on the text view in which to output orientation information and field of view information
	 */
	private TextView mFOVView;

	/**
	 * how the display is rotated. will be Surface.ROTATION_0 to Surface.ROTATION_270
	 */
	...
	@SuppressLint("NewApi")
	private void init() {
		// determine rotation of the display, which determines how we rotate our components.
	...
	/**
	 * With rotationMatrix, rotate the view components - we are assuming a fixed camera looking down the origin, and rotating the world around us.
	...
	public void rotateView(Matrix rotationMatrix) {		
		...
		// Reset all components to their starting values, with each update we calculate anew
		...
		if (!mPerspectiveVertexBatch.isEmpty()) {	
			...
			// this maps a value into vector.w; x, y are divided by w to produce perspective corrected x, y values.
			// the range of z is arbitrarily specified by us and depends on how much resolution we want.
			//
			// We multiply by our rotation matrix.  Matrix multiplication is done in reverse order when represented this way.  Perspective is logically last.
			//
			// Then we scale and translate:
			// add 1, and mult by 0.5, width / height - this translates 0,0 to 0.5, 0.5, moving the origin to the center of the view
			//
			// For 0, 90, 270, we make the assumption that our rotation matrix is already taking this device position into account.
			// this is achieved by re-mapping the sensor coordinate systems in these cases.
			...
			// TODO: find a way to eliminate the need for this branch? I was not able to successfully re-map the sensor coordinate systems to accomplish this.
		...
		if (!mOrthographicVertexBatch.isEmpty()) {
			...
			// Then we scale and translate, with the origin at the center of the device display
			// Here, orthographic scale is simply how "zoomed in" we are to the orthographic view
			//
			// For 0, 90, 270, we make the assumption that our rotation matrix is already taking this device position into account.
			// this is achieved by re-mapping the sensor coordinate systems in these cases.
			switch (mDisplayRotation) {
		...
		// prepare for drawing - ask components to prepare for drawing; publish any modified values.
		synchronized (mPublishingLock) {
			for (DrawingComponent drawingComponent : mDisplayList) {
				drawingComponent.prepareDraw();
			}
			
			// derive the orientation of the device based on where the back of the phone is pointing, from our rotated points
			// TODO: find some more efficient/correct implementation of doing this.
	...
	/**
	 * Calculate the smallest drawing radius and aspect ratio when this is called by the system during layout
	 */
	@Override
	...
	/**
	 * updates our perspective projection matrix with the current field of view and view aspect ratio
	 */
