...
public class RotationVectorCompass extends Activity implements RotationUpdateDelegate, CameraPreviewLayer.FOVUpdateDelegate {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mScaleAndFOVSlider.setProgress(
        		mPerspectiveProjection ?
        			(int)((mFOV - MIN_FOV)/(MAX_FOV-MIN_FOV) * 1000f)
        		:
        			(int)((mOrthographicScale - MIN_ORTHO_SCALE)/(MAX_ORTHO_SCALE-MIN_ORTHO_SCALE) * 1000f)
        );
	...
	private void startCamera() {
		...
		try {
			if (mDefaultCameraID != -1) {
		        ...
		        // create preview and attach to view -- will be removed when pausing, will be recreated when resuming
	...
	public void onRotationUpdate(float[] newMatrix) {
		// remap matrix values according to display rotation, as in SensorManager documentation.
