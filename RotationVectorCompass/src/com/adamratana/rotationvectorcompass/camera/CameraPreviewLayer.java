...
public class CameraPreviewLayer extends ViewGroup implements SurfaceHolder.Callback {
    ...
    public void setCanAutoFocus(boolean b) { mCanAutoFocus = b; }	
    ...
    /**
     * Updates the reported field of view based on the aspect ratio of the surface, calls back to the FOVDelegate
     * @param width
     * @param height
     */
	...
	/**
	 * Handle screen touch events, in this case, focus when we touch, stop when we let go
	 * @param ev
	 * @return
	 */
    ...
    /**
     * Given a camera, setup a surface to display the preview optimally, layout accordingly
     * @param camera
     */
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
    	// When layout happens, find the right size for the camera preview, based on the current width and height of the display (which can be either portrait or landscape)
    	log("onLayout()");
        if ((mForceLayout || changed) && getChildCount() > 0) {
            ...
            // accounting for rotated camera preview - the preview itself is always w > h due to physical sensor
            if (mPreviewSize != null) {
            	if (w < h) {
	...
	public void surfaceCreated(SurfaceHolder holder) {
        ...
        // important for autofocus, we can't allow autofocus until the camera is attached to the surface.
    	mHasSurface = true;
    }
	...
	/**
	 * Delegate for receiving updates to the camera field of view 
	 * Updates are necessary when the screen rotates, the vertical field of view which projections are based on will become the horizontal field of view of the camera sensor, for instance
	 * @author Adam
	 *
	 */
	public interface FOVUpdateDelegate {
		 ...
		 * @param width - width of the display surface
		 * @param height - height of the display surface
		 * @param fovH - original camera reported horizontal field of view
		 * @param fovV - original camera reported vertical field of view
		 * @param adjustedFOVH - adjusted horizontal field of view
		 * @param adjustedFOVV - adjusted vertical field of view
		 */
